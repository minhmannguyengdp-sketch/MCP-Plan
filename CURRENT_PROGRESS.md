...
> Trạng thái: **ROUTE -> ACTIVE SESSION HOTFIX MERGED + DB VERIFIED — VERCEL/VPS/UI/F05 RUNTIME PENDING**

## 1. Điểm tiếp tục duy nhất

```text
1. Vercel production deploy merge SHA 5276abc8... hiện bị build-rate-limit; deploy lại khi quota cho phép.
2. Chạy VPS pullmcp; không đụng milktea-backend port 3002.
3. Smoke Gateway route-customer add execute/replay/conflict/audit.
4. Chuẩn hóa fixture hoặc session state để có đúng một active session rồi UI smoke cả hai lựa chọn.
5. Regression UI Thêm khách trong Phiên.
6. Chạy F05 runtime closure smoke => PASS + fixtureCleanup=PASS.
...
Vercel production:     BLOCKED — build-rate-limit
VPS pullmcp:           PENDING
...
Vercel production:   READY — source đã deploy trước PR #29
UI functional smoke: PENDING
```
...
```text
Route master -> active session source + DB + merge             PASS
Route master -> active session Vercel/VPS/UI smoke             PENDING
