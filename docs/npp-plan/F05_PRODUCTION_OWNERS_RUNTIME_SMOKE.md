...
Every persisted-idempotent mutation must prove first execution, same-key replay, different-payload conflict, canonical envelope, exact persisted execute/replay request IDs, trusted context, one completed idempotency record, append-only succeeded/replayed audit events, and its business invariant. Archive evidence is separate: intent, R2/delete job, retry/reclaim, and finalizer must each pass. The tool must never describe PostgreSQL and R2 as one transaction.

## Mandatory guard

The command refuses to start unless all values are explicitly present and identities match. R2 capability is preflighted before the first temporary fixture mutation:
...
* Cleanup runs from `finally`, archives only captured temporary route IDs, and then checks temporary rows, outlet-media references, and the exact captured R2 object are absent.
