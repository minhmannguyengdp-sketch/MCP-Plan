...
replaceMany("src/app/mcp-setting/groups/page.tsx", [
  ['import { PageHeader } from "@/ui/layout/PageHeader";', 'import { PageHeader } from "@/ui/layout/PageHeader";\nimport { userFacingError } from "@/lib/ui/user-facing-error";'],
...
replaceMany("src/features/market-checks/MarketChecksClientPage.tsx", [
  ...
  ['title={group ? `Kết quả thử sản phẩm · ${group.routeName}` : "Kết quả thử sản phẩm"}', 'title={group ? `Kết quả thử sản phẩm · ${group.routeName}` : "Kết quả thử sản phẩm"}'],
...
write(
  "scripts/audit-user-facing-copy.mjs",
  `import fs from "node:fs";
import path from "node:path";
