...
async function resolveSessionCustomerId(input: string, body: Dict) {
  ...
  const routeCustomerId = text(body.routeCustomerId || body.route_customer_id || context.routeCustomerId || context.route_customer_id);
  ...
  const sessionDate = text(body.sessionDate || body.session_date || body.date || context.sessionDate || context.session_date || context.sessionDate || context.date)?.slice(0, 10);
...
export async function POST(request: Request) {
  try {
    ...
    const data = await callReportRpc({
      ...
      p_raw_payload: { context: isObj(body.context) ? body.context : {}, fields, selected: { competitors, usedProducts, settingItems: allItems }, inputSessionCustomerId, resolvedSessionCustomerId: sessionCustomerId },
