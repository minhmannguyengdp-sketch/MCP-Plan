...
export async function GET(request: Request) {
  try {
    ...
    const titleDate = params.get("date") || dateText(rows[0]?.report_date) || "gần nhất";
    ...
    <h2>Chi tiết báo cáo</h2>${table<Row>([
      { header: "Ngày", value: (r) => dateText(r.report_date) },
      { header: "Sale", value: (r) => r.sales },
      { header: "Tuyến", value: (r) => r.route_name },
      { header: "Khách", value: (r) => rawText(r, "context.customerName") },
      { header: "Đối thủ", value: (r) => rawText(r, "selected.competitors") || r.competitor_summary },
      { header: "SP/brand đang dùng", value: (r) => rawText(r, "selected.usedProducts") || r.company_product_summary },
      { header: "Giá", value: (r) => r.price_summary },
      { header: "Nhu cầu", value: (r) => r.demand_summary },
