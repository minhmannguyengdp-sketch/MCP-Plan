...
export default async function McpSessionsPage({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  return <AppShell activeHref="/mcp/sessions"><div className="mcp-sessions-page"><PageHeader eyebrow="MCP" title="Phiên chạy tuyến" subtitle="Tra cứu các phiên đi tuyến theo ngày, tuyến và trạng thái."><ExportMenu label="Xuất danh sách" primary groups={[{ title: "Excel theo bộ lọc", links: [buildExportLink("Danh sách điểm bán trong phiên", `/api/backend/exports/mcp-sessions.csv${suffix}`, "primary", "Theo tuyến/trạng thái đang lọc"), buildExportLink("Đơn hàng", "/api/backend/exports/orders.csv"), buildExportLink("Báo cáo thị trường", "/api/backend/exports/market-reports.csv"), buildExportLink("Việc cần theo dõi", "/api/backend/exports/followups.csv")] }]} /></PageHeader><McpSessionsManagerSafe data={data} filters={filters} /></div></AppShell>;
}
