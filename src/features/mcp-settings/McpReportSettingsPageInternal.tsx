...
function copyHint(k: string) { if (k === "competitor") return { title: "Thêm đối thủ", label: "Tên đối thủ", labelPh: "Nhập tên đối thủ hoặc nguồn hàng cạnh tranh", value: "Ghi chú ngắn", valuePh: "Ví dụ: giá thấp, chiết khấu cao, phủ kệ tốt" }; if (k === "field") return { title: "Thêm ghi chú nhanh", label: "Nội dung lựa chọn", labelPh: "Ví dụ: cần báo giá, thiếu hàng, muốn thử sản phẩm", value: "Gợi ý", valuePh: "Nội dung hiển thị khi ghi nhận thị trường" }; return { title: "Nhóm sản phẩm đang dùng", label: "Tên thương hiệu hoặc sản phẩm", labelPh: "Nhập thương hiệu hoặc sản phẩm", value: "Ghi chú", valuePh: "Thông tin bổ sung khi cần" }; }
...
export function McpReportSettingsPageInternal({ activeHref = "/mcp-setting" }: { activeHref?: string }) {
  ...
  async function load() { setLoading(true); setMsg(null); try { const payload = await json(`${API}?groupType=market_report&includeInactive=1`); const next = (payload.data?.groups || []) as Group[]; setGroups(next); setGroupId((cur) => cur || next[0]?.id || ""); } catch (e) { setMsg(userFacingError(e, "Không tải được cài đặt báo cáo. Vui lòng thử lại.")); } finally { setLoading(false); } }
  useEffect(() => { void load(); }, []);
  ...
  return <AppShell activeHref={activeHref}>
    ...
    <section className="mcp-gate-banner"><strong>Nguyên tắc</strong><span>Các lựa chọn đang sử dụng sẽ xuất hiện trong biểu mẫu ghi nhận thị trường. Có thể tắt mục ít dùng mà không làm mất dữ liệu cũ.</span></section>
