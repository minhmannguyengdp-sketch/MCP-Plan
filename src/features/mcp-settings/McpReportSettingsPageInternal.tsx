...
function copyHint(k: string) { if (k === "competitor") return { title: "Tạo chip đối thủ", label: "Tên đối thủ để sales tick", labelPh: "VD: Mama / Golden Farm / Thu Hương", value: "Ghi chú ngắn", valuePh: "VD: giá thấp, đang phủ kệ, chiết khấu mạnh" }; if (k === "field") return { title: "Tạo chip câu hỏi nhanh", label: "Nội dung cần khảo sát", labelPh: "VD: Có nhu cầu test vị / Cần báo giá / Thiếu hàng", value: "Gợi ý khi tick", valuePh: "VD: tick nếu khách có nhắc tới trong lúc ghé" }; return { title: "Tạo chip sản phẩm đang dùng", label: "Tên brand/dòng hàng khách đang dùng", labelPh: "VD: Vina / Berrino / Siro chợ / Sữa A", value: "Ghi chú ngắn", valuePh: "VD: đang dùng ổn, muốn đổi nguồn, so giá" }; }
...
export function McpReportSettingsPageInternal({ activeHref = "/mcp-setting" }: { activeHref?: string }) {
  ...
  return <AppShell activeHref={activeHref}>
    <PageHeader eyebrow="MCP Setting" title="Mẫu tick báo cáo thị trường" subtitle="Tạo sẵn các chip để sales đi tuyến chỉ cần tick nhanh, hạn chế gõ ngoài đường." />
    <section className="mcp-gate-banner"><strong>Nguyên tắc</strong><span>Anh tạo mục tiêu khảo sát thủ công ở đây. Khi sales bấm BC trong phiên, hệ thống hiện chip đang bật để tick nhanh.</span></section>
