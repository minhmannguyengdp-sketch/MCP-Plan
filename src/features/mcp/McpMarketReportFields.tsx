...
export type MarketReportFieldValues = {
  priceSummary: string;
...
export type ReportSettingSelection = {
  id: string;
  key: string;
  label: string;
  value: string;
...
export type MarketReportDraft = {
  fields: MarketReportFieldValues;
  selectedCompetitors: ReportSettingSelection[];
  selectedUsedProducts: ReportSettingSelection[];
  selectedSettingItems: ReportSettingSelection[];
};
...
const EMPTY_FIELDS: MarketReportFieldValues = {
  priceSummary: "",
  displaySummary: "",
  stockSummary: "",
  demandSummary: "",
...
function isFieldGroup(group: Pick<ApiGroup, "key" | "title">) {
  const text = normalize(`${group.key} ${group.title}`);
  return text.includes("report_fields") || text.includes("field bao cao") || text.includes("field");
}

function isCompetitorGroup(group: Pick<ApiGroup, "key" | "title"> | ReportSettingSelection) {
  const text = "groupKey" in group ? normalize(`${group.groupKey} ${group.groupTitle}`) : normalize(`${group.key} ${group.title}`);
...
async function loadGroups() {
  ...
  const response = await fetch("/api/mcp-report-settings?groupType=market_report", { cache: "no-store", headers: { Accept: "application/json" } });
...
export function McpMarketReportFields({ value, onChange, saving }: { value: MarketReportDraft; onChange: (value: MarketReportDraft) => void; saving: boolean }) {
  ...
  useEffect(() => {
    ...
    loadGroups()
      ...
      .catch((err) => {
        if (active) setError(err instanceof Error ? err.message : "Không tải được MCP Setting");
      })
      .finally(() => {
  ...
  function toggle(group: ApiGroup, item: ApiItem) {
    const picked = selectionFrom(group, item);
    ...
    onChange({
      ...
      selectedUsedProducts: nextAll.filter((selected) => !isCompetitorGroup(selected))
    });
  }

  return (
    <div className="grid">
      <section className="card report-quick-panel" style={{ padding: 10, display: "grid", gap: 10 }}>
        <div>
          <strong>Tick nhanh báo cáo</strong>
          <p className="page-subtitle" style={{ margin: "4px 0 0" }}>Lưu kèm ID mẫu vào raw_payload.</p>
        </div>
        {loading ? <p className="page-subtitle">Đang tải mẫu báo cáo...</p> : null}
        {error ? <p className="page-subtitle order-message">{error}</p> : null}
        {settingGroups.map((group) => (
          <div className="report-quick-group" key={group.id}>
            <strong>{group.title}</strong>
            <div className="mcp-status-chips">
              {group.items.map((item) => (
                <label className="button" key={item.id} style={{ display: "inline-flex", gap: 6, alignItems: "center" }}>
                  <input type="checkbox" checked={selectedIds.has(item.id)} onChange={() => toggle(group, item)} disabled={saving} />
                  {item.label}
                </label>
              ))}
            </div>
        ...
        {!loading && settingGroups.length === 0 ? <p className="page-subtitle">Chưa có mẫu tick nhanh đang bật.</p> : null}
      </section>

      <section className="card report-quick-panel" style={{ padding: 10, display: "grid", gap: 10 }}>
        <div>
          <strong>Field báo cáo</strong>
          <p className="page-subtitle" style={{ margin: "4px 0 0" }}>Phần trống sẽ không lưu vào summary.</p>
        </div>
        <div className="grid">
          {FIELD_DEFS.map((field) => (
            <label className="form-field" key={field.key}>
              <small>{field.label}</small>
              <textarea value={value.fields[field.key]} onChange={(event) => updateField(field.key, event.target.value)} disabled={saving} />
            </label>
          ))}
        </div>
      </section>
