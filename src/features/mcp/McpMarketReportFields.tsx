...
function SelectedSummary({ draft }: { draft: MarketReportDraft }) {
  ...
  return <div className="report-selected-summary"><strong>Đã chọn</strong><span>{selected.map((item) => item.label).join(", ")}{fieldCount ? `${selected.length ? " · " : ""}${fieldCount} ô ghi thêm` : ""}</span></div>;
}
