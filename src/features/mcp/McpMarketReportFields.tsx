...
function slug(value: string) { return norm(value).replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "") || "item"; }
...
function usedProductSelection(category: string, label: string): ReportSettingSelection { const id = `used_${slug(category)}_${slug(label)}`; return { id, key: id, label, value: label, groupKey: `used_products_${slug(category)}`, groupTitle: category, category, brandName: label, productId: "" }; }
...
export function McpMarketReportFields({ value, onChange, saving }: { value: MarketReportDraft; onChange: (value: MarketReportDraft) => void; saving: boolean }) {
  ...
  useEffect(() => { let active = true; loadGroups().then((nextGroups) => { if (!active) return; setGroups(nextGroups || []); setError(null); }).catch((err) => { if (active) setError(err instanceof Error ? err.message : "Không tải được dữ liệu tick"); }).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, []);
  ...
  return <div className="report-popup-grid"><section className="report-quick-panel"><div className="report-section-head"><strong>Tick nhanh báo cáo</strong><small>Tick theo nhóm sản phẩm rồi chọn thương hiệu khách đang dùng.</small></div>{loading ? <p className="page-subtitle">Đang tải dữ liệu tick...</p> : null}{error ? <p className="page-subtitle order-message">{error}</p> : null}{competitorGroups.map((group) => <div className="report-quick-group" key={group.id}><strong>Đối thủ</strong><div className="report-chip-grid">{group.items.map((item) => <label className={selectedIds.has(item.id) ? "report-chip selected" : "report-chip"} key={item.id}><input type="checkbox" checked={selectedIds.has(item.id)} onChange={() => toggle(group, item)} disabled={saving} />{item.label}</label>)}</div></div>)}<div className="report-quick-group"><strong>Thương hiệu/sản phẩm khách đang dùng</strong><div className="report-used-product-groups">{USED_PRODUCT_GROUPS.map((group) => <div className="report-used-product-group" key={group.title}><span>{group.title}</span><div className="report-chip-grid">{group.items.map((label) => { const item = usedProductSelection(group.title, label); return <label className={selectedIds.has(item.id) ? "report-chip selected" : "report-chip"} key={item.id}><input type="checkbox" checked={selectedIds.has(item.id)} onChange={() => applySelection(item)} disabled={saving} />{label}</label>; })}</div></div>)}</div></div><div className="report-quick-group"><strong>Ghi chú nhanh</strong><div className="report-chip-grid">{QUICK_NOTE_ITEMS.map((label) => { const active = value.fields.note.includes(label); return <button className={active ? "report-chip selected" : "report-chip"} key={label} type="button" onClick={() => toggleQuickNote(label)} disabled={saving}>{label}</button>; })}</div></div></section><section className="report-field-panel"><div className="report-section-head"><strong>Ghi thêm</strong><small>Ô trống sẽ không lưu vào summary.</small></div><div className="report-field-grid">{FIELD_DEFS.map((field) => <label className="form-field report-field" key={field.key}><small>{field.label}</small><textarea rows={field.compact ? 2 : 2} value={value.fields[field.key]} onChange={(event) => updateField(field.key, event.target.value)} disabled={saving} /></label>)}</div></section></div>;
}
