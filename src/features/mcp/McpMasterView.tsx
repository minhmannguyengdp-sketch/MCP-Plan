...
export function McpMasterView({ activeHref, routesData, routeCustomersData }: { activeHref: string; routesData: RoutesData; routeCustomersData: RouteCustomersData }) {
  ...
  return (
    <AppShell activeHref={activeHref}>
      <PageHeader eyebrow="Tuyến MCP" title="Tuyến MCP" subtitle="Chọn tuyến trước, rồi xem khách tuyến.">
      ...
      <section className="dashboard-section">
        <div className="dashboard-section-head"><h2>Tuyến MCP</h2><span>{selectedRoute ? `Đã chọn: ${selectedRoute.name}` : "Chưa chọn tuyến"}</span></div>
        <div className="mcp-status-chips" role="tablist" aria-label="Tuyến MCP">
          ...
          <button className={tab === "customers" ? "active" : ""} type="button" onClick={() => setTab("customers")}>Khách tuyến <b>{selectedRoute ? selectedCustomers.length : 0}</b></button>
          <button className={tab === "gps" ? "active" : ""} type="button" onClick={() => setTab("gps")}>Cần GPS <b>{selectedRoute ? gpsCustomers.length : 0}</b></button>
          <button className={tab === "open" ? "active" : ""} type="button" onClick={() => setTab("open")}>Chuẩn bị phiên <b>{openRoutes.length}</b></button>
        </div>
      </section>

      {tab === "routes" ? routesData.routes.length > 0 ? <div className="mcp-line-list">{routesData.routes.map((route) => <RouteCard key={route.id} route={route} selected={selectedRoute?.id === route.id} onSelect={chooseRoute} />)}</div> : <EmptyPanel title="Chưa có tuyến" hint="Chưa có dữ liệu tuyến MCP." /> : null}
      ...
      {tab === "open" ? openRoutes.length > 0 ? <div className="mcp-line-list">{openRoutes.map((route) => <RouteCard key={route.id} route={route} selected={selectedRoute?.id === route.id} onSelect={chooseRoute} />)}</div> : <EmptyPanel title="Không có tuyến đang chạy" hint="Chỉ tuyến đang chạy hoặc cần theo dõi mới dùng để chuẩn bị phiên." /> : null}
