...
function CustomerEditSheet({ mode, customer, route, draft, saving, message, onDraftChange, onUseCurrentLocation, onClose, onSubmit }: { mode: CustomerEditorMode | null; customer: RouteCustomerItem | null; route: RouteItem | null; draft: CustomerDraft; saving: boolean; message: string | null; onDraftChange: (field: keyof CustomerDraft, value: string | boolean) => void; onUseCurrentLocation: () => void; onClose: () => void; onSubmit: () => void }) {
  const open = Boolean(mode);
  const isDelete = mode === "delete";
  const title = mode === "create" ? "Thêm điểm bán vào tuyến" : mode === "edit" ? "Sửa điểm bán" : "Xóa điểm bán";
  ...
  return <BottomSheet open={open} onClose={onClose} title={title} description={isDelete ? `${customer?.accountName || "Điểm bán"} · xóa khỏi tuyến và dữ liệu liên quan` : `Tuyến: ${routeName}`} footer={<div className="sheet-action-grid"><button className="button primary" type="button" onClick={onSubmit} disabled={saving}>{submitLabel}</button><button className="button" type="button" onClick={onClose} disabled={saving}>Đóng</button></div>}>
