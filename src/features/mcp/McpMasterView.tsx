...
function RouteEditSheet({ mode, route, draft, saving, message, onDraftChange, onClose, onSubmit }: { mode: RouteEditorMode | null; route: RouteItem | null; draft: RouteDraft; saving: boolean; message: string | null; onDraftChange: (field: keyof RouteDraft, value: string | boolean) => void; onClose: () => void; onSubmit: () => void }) {
  ...
  return <BottomSheet open={open} onClose={onClose} title={title} description={isDelete && route ? `${route.name} · xóa thật, không archive` : "Tuyến bán hàng dùng để quản lý danh sách điểm bán và mở phiên theo ngày."} footer={<div className="sheet-action-grid"><button className="button primary" type="button" onClick={onSubmit} disabled={saving}>{submitLabel}</button><button className="button" type="button" onClick={onClose} disabled={saving}>Đóng</button></div>}>
...
export function McpMasterView({ activeHref, routesData, routeCustomersData }: { activeHref: string; routesData: RoutesData; routeCustomersData: RouteCustomersData }) {
  ...
  function openCustomerCreate() { if (!selectedRoute) { setMessage("Cần chọn tuyến gốc trước khi thêm khách"); return; } setCustomerEditorMode("create"); setCustomerEditorCustomer(null); setCustomerDraft(emptyCustomerDraft(selectedRoute)); setSelectedCustomer(null); setMessage(null); }
  ...
  function useCurrentLocation() {
    ...
    navigator.geolocation.getCurrentPosition((pos) => { const lat = pos.coords.latitude; const lng = pos.coords.longitude; const accuracy = pos.coords.accuracy; setCustomerDraft((current) => ({ ...current, geoLat: String(lat), geoLng: String(lng), geoAccuracy: Number.isFinite(accuracy) ? String(Math.round(accuracy)) : current.geoAccuracy })); setMessage(`Đã lấy định vị: ${lat.toFixed(6)}, ${lng.toFixed(6)}. Bấm Lưu khách để ghi vào tuyến.`); }, (error) => { setMessage(error.message || "Không lấy được định vị, kiểm tra quyền GPS của trình duyệt"); }, { enableHighAccuracy: true, timeout: 15000, maximumAge: 0 });
  }

  function submitRouteEditor() { if (!routeEditorMode) return; startSaving(async () => { try { setMessage(null); if (routeEditorMode === "create") { if (!routeDraft.routeName.trim()) throw new Error("Cần nhập tên tuyến"); const response = await fetch("/api/routes", { method: "POST", headers: { Accept: "application/json", "Content-Type": "application/json" }, body: JSON.stringify(routeDraft) }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error || "Không tạo được tuyến"); } else if (routeEditorMode === "edit" && routeEditorRoute) { if (!routeDraft.routeName.trim()) throw new Error("Cần nhập tên tuyến"); const response = await fetch(`/api/routes/${encodeURIComponent(routeEditorRoute.id)}`, { method: "PATCH", headers: { Accept: "application/json", "Content-Type": "application/json" }, body: JSON.stringify(routeDraft) }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error || "Không sửa được tuyến"); } else if (routeEditorMode === "delete" && routeEditorRoute) { const response = await fetch(`/api/routes/${encodeURIComponent(routeEditorRoute.id)}/archive`, { method: "POST", headers: { Accept: "application/json" } }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error || "Không xóa được tuyến"); if (selectedRoute?.id === routeEditorRoute.id) setSelectedRoute(null); } setRouteEditorMode(null); setRouteEditorRoute(null); router.refresh(); } catch (error) { setMessage(error instanceof Error ? error.message : "Không lưu được tuyến"); } }); }
  function submitCustomerEditor() { if (!customerEditorMode) return; startSaving(async () => { try { setMessage(null); if (customerEditorMode === "create") { if (!selectedRoute) throw new Error("Cần chọn tuyến gốc"); if (!customerDraft.customerName.trim()) throw new Error("Cần nhập tên khách"); const response = await fetch("/api/route-customers", { method: "POST", headers: { Accept: "application/json", "Content-Type": "application/json" }, body: JSON.stringify({ ...customerDraft, geoSource: customerDraft.geoLat && customerDraft.geoLng ? "browser" : undefined, routeId: selectedRoute.id }) }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error || "Không thêm được khách tuyến"); } else if (customerEditorMode === "edit" && customerEditorCustomer) { if (!customerDraft.customerName.trim()) throw new Error("Cần nhập tên khách"); const response = await fetch(`/api/route-customers/${encodeURIComponent(customerEditorCustomer.id)}`, { method: "PATCH", headers: { Accept: "application/json", "Content-Type": "application/json" }, body: JSON.stringify({ ...customerDraft, geoSource: customerDraft.geoLat && customerDraft.geoLng ? "browser" : undefined }) }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error || "Không sửa được khách tuyến"); } else if (customerEditorMode === "delete" && customerEditorCustomer) { const response = await fetch(`/api/route-customers/${encodeURIComponent(customerEditorCustomer.id)}/archive`, { method: "POST", headers: { Accept: "application/json" } }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error || "Không xóa được khách tuyến"); if (selectedCustomer?.id === customerEditorCustomer.id) setSelectedCustomer(null); } setCustomerEditorMode(null); setCustomerEditorCustomer(null); router.refresh(); } catch (error) { setMessage(error instanceof Error ? error.message : "Không lưu được khách tuyến"); } }); }
  function openSession(route: RouteItem) { const sessionDate = todayDateOnly(); setSelectedRoute(route); setMessage(null); startSaving(async () => { try { const response = await fetch("/api/backend/mcp-day/open-session", { method: "POST", headers: { Accept: "application/json", "Content-Type": "application/json" }, body: JSON.stringify({ routeId: route.id, sessionDate, owner: route.salesOwner || "Sale" }) }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error || "Không mở được phiên MCP"); router.push(`/visits?routeId=${encodeURIComponent(route.id)}&date=${encodeURIComponent(sessionDate)}`); } catch (error) { setMessage(error instanceof Error ? error.message : "Không mở được phiên MCP"); } }); }

  return <AppShell activeHref={activeHref}><PageHeader eyebrow="Tuyến bán hàng" title="Tuyến bán hàng" subtitle="Quản lý master tuyến: tạo, sửa, xóa dứt điểm, quản lý khách rồi mở phiên theo ngày."><button className="button primary" type="button" onClick={openRouteCreate}>Tạo tuyến</button></PageHeader><FilterBar filters={[{ label: "Tuyến", value: String(routesData.routes.length) }, { label: "Đang chọn", value: selectedRoute?.name ?? "Chưa chọn" }, { label: "Điểm bán", value: selectedRoute ? String(selectedCustomers.length) : "-" }, { label: "Cần GPS", value: selectedRoute ? String(gpsCustomers.length) : "-" }]} />{message && !routeEditorMode && !customerEditorMode ? <section className="empty-inline"><strong>{message}</strong></section> : null}<section className="dashboard-section"><div className="dashboard-section-head"><h2>Tuyến bán hàng</h2><span>{selectedRoute ? `Đã chọn: ${selectedRoute.name}` : "Chưa chọn tuyến"}</span></div><div className="mcp-status-chips" role="tablist" aria-label="Tuyến bán hàng"><button className={tab === "routes" ? "active" : ""} type="button" onClick={() => setTab("routes")}>Tuyến <b>{routesData.routes.length}</b></button><button className={tab === "customers" ? "active" : ""} type="button" onClick={() => setTab("customers")}>Điểm bán <b>{selectedRoute ? selectedCustomers.length : 0}</b></button><button className={tab === "gps" ? "active" : ""} type="button" onClick={() => setTab("gps")}>Cần GPS <b>{selectedRoute ? gpsCustomers.length : 0}</b></button><button className={tab === "open" ? "active" : ""} type="button" onClick={() => setTab("open")}>Chuẩn bị phiên <b>{openRoutes.length}</b></button></div></section>{tab === "routes" ? routesData.routes.length > 0 ? <div className="mcp-line-list">{routesData.routes.map((route) => <RouteCard key={route.id} route={route} selected={selectedRoute?.id === route.id} saving={saving} onSelect={chooseRoute} onOpenSession={openSession} onEdit={openRouteEdit} onDelete={openRouteDelete} />)}</div> : <EmptyPanel title="Chưa có tuyến" hint="Bấm Tạo tuyến để thêm tuyến gốc mới." /> : null}{tab === "customers" ? !selectedRoute ? <EmptyPanel title="Chọn một tuyến" hint="Vui lòng chọn tuyến gốc trước, rồi hệ thống mới hiện khách thuộc tuyến đó." /> : <div className="mcp-line-list"><button className="button primary" type="button" onClick={openCustomerCreate} disabled={saving}>Thêm khách vào tuyến</button>{selectedCustomers.length > 0 ? selectedCustomers.map((customer) => <CustomerCard key={customer.id} customer={customer} onSelect={setSelectedCustomer} onEdit={openCustomerEdit} onDelete={openCustomerDelete} />) : <EmptyPanel title="Tuyến chưa có khách" hint="Bấm Thêm khách vào tuyến để tạo khách gốc." />}<button className="button primary" type="button" disabled={saving} onClick={() => openSession(selectedRoute)}>{saving ? "Đang mở phiên..." : "Mở phiên hôm nay"}</button></div> : null}{tab === "gps" ? !selectedRoute ? <EmptyPanel title="Chọn một tuyến" hint="Vui lòng chọn tuyến trước để xem khách cần GPS." /> : gpsCustomers.length > 0 ? <div className="mcp-line-list">{gpsCustomers.map((customer) => <CustomerCard key={customer.id} customer={customer} onSelect={setSelectedCustomer} onEdit={openCustomerEdit} onDelete={openCustomerDelete} />)}</div> : <EmptyPanel title="GPS đã ổn" hint="Tuyến đang chọn không có khách cần bổ sung GPS." /> : null}{tab === "open" ? openRoutes.length > 0 ? <div className="mcp-line-list">{openRoutes.map((route) => <RouteCard key={route.id} route={route} selected={selectedRoute?.id === route.id} saving={saving} onSelect={chooseRoute} onOpenSession={openSession} onEdit={openRouteEdit} onDelete={openRouteDelete} />)}</div> : <EmptyPanel title="Không có tuyến đang chạy" hint="Chỉ tuyến đang chạy hoặc cần theo dõi mới dùng để chuẩn bị phiên." /> : null}<CustomerSheet customer={selectedCustomer} onClose={() => setSelectedCustomer(null)} /><RouteEditSheet mode={routeEditorMode} route={routeEditorRoute} draft={routeDraft} saving={saving} message={message} onDraftChange={updateRouteDraft} onClose={closeRouteEditor} onSubmit={submitRouteEditor} /><CustomerEditSheet mode={customerEditorMode} customer={customerEditorCustomer} route={selectedRoute} draft={customerDraft} saving={saving} message={message} onDraftChange={updateCustomerDraft} onUseCurrentLocation={useCurrentLocation} onClose={closeCustomerEditor} onSubmit={submitCustomerEditor} /></AppShell>;
}
