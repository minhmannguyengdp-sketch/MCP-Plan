...
function customerToDraft(customer: RouteCustomerItem): CustomerDraft {
  return {
    ...
    geoAccuracy: customer.gps?.accuracyMeters ? String(customer.gps.accuracyMeters) : ""
  };
}
...
function EmptyPanel({ title, hint }: { title: string; hint: string }) {
  return (
    <div className="empty-inline">
      <strong>{title}</strong>
      <p className="page-subtitle">{hint}</p>
    </div>
  );
}
...
function CustomerSheet({
  ...
  customer: RouteCustomerItem | null;
  onClose: () => void;
}) {
  ...
  return (
    <BottomSheet
      ...
      onClose={onClose}
      title={customer ? customer.accountName : "Điểm bán"}
      ...
      {customer ? (
        <div className="outlet-sheet-content">
          ...
          <div className="grid">
            ...
            <div className="metric-row"><span>Cập nhật vị trí</span><strong>{customer.gps?.updatedAt ?? "Chưa có"}</strong></div>
          </div>
          <RouteCustomerMediaPreview
            key={customer.id}
...
function RouteEditSheet({
  ...
  mode: RouteEditorMode | null;
  route: RouteItem | null;
  draft: RouteDraft;
  saving: boolean;
  message: string | null;
  onDraftChange: (field: keyof RouteDraft, value: string | boolean) => void;
  onClose: () => void;
  onSubmit: () => void;
}) {
  ...
  return (
    <BottomSheet
      ...
      onClose={onClose}
      title={title}
      description={isDelete && route ? `${route.name} · xóa toàn bộ dữ liệu liên quan` : "Tuyến bán hàng dùng để quản lý danh sách điểm bán và mở phiên theo ngày."}
      footer={
        <div className="sheet-action-grid">
          <button className="button primary" type="button" onClick={onSubmit} disabled={saving}>{submitLabel}</button>
    ...
    >
      {isDelete ? (
        <div className="visit-sheet-content">
          <div className="visit-focus-card"><span>Cảnh báo</span><strong>Xóa khỏi hệ thống</strong><small>Tuyến, danh sách điểm bán và toàn bộ dữ liệu phát sinh liên quan sẽ bị xóa. Dữ liệu đã xóa không thể khôi phục.</small></div>
...
function CustomerEditSheet({
  ...
  onClose,
  onSubmit,
  onIncludeActiveSession,
  onRouteOnly,
  onCancelActiveSessionPrompt
}: {
  mode: CustomerEditorMode | null;
  customer: RouteCustomerItem | null;
  route: RouteItem | null;
  draft: CustomerDraft;
  saving: boolean;
  ...
  onDraftChange: (field: keyof CustomerDraft, value: string | boolean) => void;
  onUseCurrentLocation: () => void;
  onClose: () => void;
  onSubmit: () => void;
  onIncludeActiveSession: () => void;
  onRouteOnly: () => void;
  onCancelActiveSessionPrompt: () => void;
}) {
  const open = Boolean(mode);
  const isDelete = mode === "delete";
  ...
  const footer = activeSessionPrompt ? (
    <div className="sheet-action-grid">
      <button className="button primary" type="button" onClick={onIncludeActiveSession} disabled={saving}>Thêm vào tuyến và phiên</button>
      <button className="button" type="button" onClick={onRouteOnly} disabled={saving}>Chỉ thêm vào tuyến</button>
    </div>
  ) : (
    <div className="sheet-action-grid">
      <button className="button primary" type="button" onClick={onSubmit} disabled={saving}>{submitLabel}</button>
  ...
  return (
    <BottomSheet
      open={open}
      onClose={activeSessionPrompt ? onCancelActiveSessionPrompt : onClose}
      title={title}
      description={activeSessionPrompt ? `Tuyến: ${activeSessionPrompt.routeName}` : isDelete ? `${customer?.accountName || "Điểm bán"} · xóa khỏi tuyến và dữ liệu liên quan` : `Tuyến: ${routeName}`}
      footer={footer}
    >
      {activeSessionPrompt ? (
        <div className="visit-sheet-content">
          <div className="visit-focus-card"><span>Phiên đang hoạt động</span><strong>Tuyến này đang có phiên hoạt động. Thêm khách vào phiên hiện tại luôn?</strong><small>Phiên ngày {activeSessionPrompt.sessionDate}. Lựa chọn mặc định sẽ thêm điểm bán vào cả tuyến cố định và phiên đang chạy.</small></div>
          {message ? <p className="page-subtitle">{message}</p> : null}
        </div>
      ) : isDelete ? (
        <div className="visit-sheet-content">
          <div className="visit-focus-card"><span>Cảnh báo</span><strong>Xóa khỏi hệ thống</strong><small>Điểm bán và các dữ liệu liên quan trong những phiên trước sẽ bị xóa. Dữ liệu đã xóa không thể khôi phục.</small></div>
          <div className="metric-row"><span>Điểm bán</span><strong>{customer?.accountName}</strong></div>
          <div className="metric-row"><span>Tuyến</span><strong>{customer?.routeName}</strong></div>
          {message ? <p className="page-subtitle">{message}</p> : null}
        </div>
      ) : (
        <div className="visit-sheet-content">
          <label className="form-field"><small>Tên điểm bán</small><input value={draft.customerName} onChange={(event) => onDraftChange("customerName", event.target.value)} placeholder="Nhập tên điểm bán" /></label>
          <label className="form-field"><small>Số điện thoại</small><input value={draft.phone} onChange={(event) => onDraftChange("phone", event.target.value)} placeholder="Số điện thoại" /></label>
          <label className="form-field"><small>Khu vực</small><input value={draft.area} onChange={(event) => onDraftChange("area", event.target.value)} placeholder="Khu vực" /></label>
          <label className="form-field"><small>Địa chỉ</small><input value={draft.address} onChange={(event) => onDraftChange("address", event.target.value)} placeholder="Địa chỉ" /></label>
          <label className="form-field"><small>Thứ tự ghé</small><input inputMode="numeric" value={draft.sortOrder} onChange={(event) => onDraftChange("sortOrder", event.target.value)} /></label>
          {mode === "edit" ? <label className="form-field"><small>Trạng thái</small><select value={draft.active ? "active" : "hidden"} onChange={(event) => onDraftChange("active", event.target.value === "active")}><option value="active">Đang trong tuyến</option><option value="hidden">Ẩn khỏi tuyến</option></select></label> : null}
          <div className="mcp-gate-banner"><strong>Định vị điểm bán</strong><span>Bấm lấy định vị trên điện thoại, chờ trình duyệt cấp quyền rồi bấm Lưu điểm bán.</span></div>
          <div className="sheet-action-grid"><button className="button primary" type="button" onClick={onUseCurrentLocation} disabled={saving}>Lấy định vị hiện tại</button>{mapsHref ? <a className="button" href={mapsHref} target="_blank" rel="noreferrer">Mở Maps</a> : null}</div>
          <label className="form-field"><small>Vĩ độ</small><input inputMode="decimal" value={draft.geoLat} onChange={(event) => onDraftChange("geoLat", event.target.value)} placeholder="10.762622" /></label>
          <label className="form-field"><small>Kinh độ</small><input inputMode="decimal" value={draft.geoLng} onChange={(event) => onDraftChange("geoLng", event.target.value)} placeholder="106.660172" /></label>
          <label className="form-field"><small>Độ chính xác GPS, mét</small><input inputMode="decimal" value={draft.geoAccuracy} onChange={(event) => onDraftChange("geoAccuracy", event.target.value)} placeholder="VD: 15" /></label>
          <label className="form-field"><small>Ghi chú</small><textarea value={draft.note} onChange={(event) => onDraftChange("note", event.target.value)} placeholder="Ghi chú khách tuyến" /></label>
          {message ? <p className="page-subtitle">{message}</p> : null}
        </div>
      )}
    </BottomSheet>
...
function RouteCard({
  ...
  onOpenSession: (route: RouteItem) => void;
  ...
  return (
    <OperationalListCard
      ...
      title={route.name}
      description={`${route.plannedCustomers} điểm bán - ${route.orderCount} đơn`}
      badge={<span className={routeStatusClass(route.status)}>{selected ? "Đã chọn" : routeStatusLabel(route.status)}</span>}
      meta={[`Đã ghé ${route.visitedCustomers}/${route.plannedCustomers}`, `Lần cuối ${route.lastVisitDate}`]}
      actions={[
        { label: selected ? "Xem điểm bán" : "Chọn tuyến", tone: "primary", onClick: () => onSelect(route) },
        { label: saving && selected ? "Đang mở..." : "Mở phiên hôm nay", tone: "primary", onClick: () => onOpenSession(route) },
        { label: "Sửa", onClick: () => onEdit(route) },
        { label: "Xóa", onClick: () => onDelete(route) }
      ]}
    />
  );
}
...
function CustomerCard({
  ...
  customer: RouteCustomerItem;
  onSelect: (customer: RouteCustomerItem) => void;
  onEdit: (customer: RouteCustomerItem) => void;
  onDelete: (customer: RouteCustomerItem) => void;
}) {
  return (
    <OperationalListCard
      ...
      description={customer.note || gpsLabel(customer)}
      badge={<span className={routeCustomerStatusClass(customer.status)}>{routeCustomerStatusLabel(customer.status)}</span>}
      meta={[gpsLabel(customer)]}
      actions={[
        { label: "Xem điểm bán", tone: "primary", onClick: () => onSelect(customer) },
        { label: customer.gps ? "Cập nhật vị trí" : "Lấy vị trí", tone: customer.gps ? undefined : "primary", onClick: () => onEdit(customer) },
        { label: "Sửa", onClick: () => onEdit(customer) },
        { label: "Xóa", onClick: () => onDelete(customer) }
      ]}
    />
  );
}

export function McpMasterView({
  activeHref,
  routesData,
  routeCustomersData
}: {
  ...
  const selectedCustomers = useMemo(
    () => selectedRoute ? routeCustomersData.customers.filter((customer) => customer.routeId === selectedRoute.id) : [],
    [routeCustomersData.customers, selectedRoute]
  );
  const gpsCustomers = useMemo(
    () => selectedCustomers.filter((customer) => customer.status === "needs_gps" || !customer.gps),
    [selectedCustomers]
  );
  ...
  function openCustomerCreate() {
    if (!selectedRoute) {
      setMessage("Cần chọn tuyến gốc trước khi thêm khách");
      return;
    }
    resetCustomerIntent();
    ...
    setCustomerDraft(emptyCustomerDraft(selectedRoute));
    setSelectedCustomer(null);
    setMessage(null);
  }

  function openCustomerEdit(customer: RouteCustomerItem) {
    resetCustomerIntent();
    setActiveSessionPrompt(null);
    setCustomerEditorMode("edit");
    setCustomerEditorCustomer(customer);
    setCustomerDraft(customerToDraft(customer));
    setSelectedCustomer(null);
    setMessage(null);
  }

  function openCustomerDelete(customer: RouteCustomerItem) {
    resetCustomerIntent();
    setActiveSessionPrompt(null);
    setCustomerEditorMode("delete");
    setCustomerEditorCustomer(customer);
    setCustomerDraft(customerToDraft(customer));
    setSelectedCustomer(null);
    setMessage(null);
  ...
  function updateCustomerDraft(field: keyof CustomerDraft, value: string | boolean) {
    if (customerEditorMode === "create") resetCustomerIntent();
    setCustomerDraft((current) => ({ ...current, [field]: value }));
  }

  function useCurrentLocation() {
    if (typeof navigator === "undefined" || !navigator.geolocation) {
      setMessage("Trình duyệt không hỗ trợ lấy định vị");
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        setCustomerDraft((current) => ({
          ...
          geoLat: String(lat),
          geoLng: String(lng),
          geoAccuracy: Number.isFinite(accuracy) ? String(Math.round(accuracy)) : current.geoAccuracy
        }));
        setMessage(`Đã lấy định vị: ${lat.toFixed(6)}, ${lng.toFixed(6)}. Bấm Lưu điểm bán để ghi vào tuyến.`);
      },
      (error) => {
        setMessage(error.message || "Không lấy được vị trí. Vui lòng kiểm tra quyền định vị của trình duyệt.");
      },
    ...
    startSaving(async () => {
      try {
        ...
        if (routeEditorMode === "create") {
          ...
          if (!response.ok) throw new Error(apiError(payload, "Không tạo được tuyến"));
          ...
          const payload = await response.json().catch(() => ({}));
          if (!response.ok) throw new Error(apiError(payload, "Không sửa được tuyến"));
          ...
          const payload = await response.json().catch(() => ({}));
          if (!response.ok) throw new Error(apiError(payload, "Không xóa được tuyến"));
        ...
        setMessage(userFacingError(error, "Không lưu được tuyến. Vui lòng thử lại."));
      }
    });
  }
  ...
  function openSession(route: RouteItem) {
    ...
    startSaving(async () => {
      try {
        const response = await idempotentMutationFetch(
          "/api/backend/mcp-day/open-session",
          {
            method: "POST",
            headers: { Accept: "application/json", "Content-Type": "application/json" },
            body: JSON.stringify({ routeId: route.id, sessionDate, owner: route.salesOwner || "Sale" })
          },
          { operation: "route-session.open" }
        );
        const payload = await response.json().catch(() => ({}));
        if (!response.ok) throw new Error(apiError(payload, "Không mở được phiên MCP"));
        router.push(`/visits?routeId=${encodeURIComponent(route.id)}&date=${encodeURIComponent(sessionDate)}`);
      } catch (error) {
        setMessage(userFacingError(error, "Không mở được phiên đi tuyến. Vui lòng thử lại."));
      }
    });
  }
  ...
  return (
    <AppShell activeHref={activeHref}>
      <PageHeader eyebrow="Tuyến bán hàng" title="Tuyến bán hàng" subtitle="Quản lý tuyến bán hàng, điểm bán trong tuyến và mở phiên đi thị trường theo ngày.">
        <button className="button primary" type="button" onClick={openRouteCreate}>Tạo tuyến</button>
      </PageHeader>
      <FilterBar filters={[
        { label: "Tuyến", value: String(routesData.routes.length) },
        { label: "Đang chọn", value: selectedRoute?.name ?? "Chưa chọn" },
        { label: "Điểm bán", value: selectedRoute ? String(selectedCustomers.length) : "-" },
        { label: "Cần định vị", value: selectedRoute ? String(gpsCustomers.length) : "-" }
      ]} />
      {message && !routeEditorMode && !customerEditorMode ? <section className="empty-inline"><strong>{message}</strong></section> : null}
      <section className="dashboard-section">
        <div className="dashboard-section-head"><h2>Tuyến bán hàng</h2><span>{selectedRoute ? `Đã chọn: ${selectedRoute.name}` : "Chưa chọn tuyến"}</span></div>
        <div className="mcp-status-chips" role="tablist" aria-label="Tuyến bán hàng">
          <button className={tab === "routes" ? "active" : ""} type="button" onClick={() => setTab("routes")}>Tuyến <b>{routesData.routes.length}</b></button>
          <button className={tab === "customers" ? "active" : ""} type="button" onClick={() => setTab("customers")}>Điểm bán <b>{selectedRoute ? selectedCustomers.length : 0}</b></button>
          <button className={tab === "gps" ? "active" : ""} type="button" onClick={() => setTab("gps")}>Cần định vị <b>{selectedRoute ? gpsCustomers.length : 0}</b></button>
          <button className={tab === "open" ? "active" : ""} type="button" onClick={() => setTab("open")}>Chuẩn bị phiên <b>{openRoutes.length}</b></button>
        </div>
      </section>
      ...
      {tab === "gps" ? !selectedRoute ? <EmptyPanel title="Chọn một tuyến" hint="Vui lòng chọn tuyến để xem các điểm bán cần bổ sung vị trí." /> : gpsCustomers.length > 0 ? <div className="mcp-line-list">{gpsCustomers.map((customer) => <CustomerCard key={customer.id} customer={customer} onSelect={setSelectedCustomer} onEdit={openCustomerEdit} onDelete={openCustomerDelete} />)}</div> : <EmptyPanel title="Vị trí đã đầy đủ" hint="Tuyến đang chọn không có điểm bán cần bổ sung vị trí." /> : null}
