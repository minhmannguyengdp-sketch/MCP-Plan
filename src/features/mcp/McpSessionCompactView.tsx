...
type ReportTemplate = { id: string; title: string; reportType?: string; scopeType?: string; content?: string; priceSummary?: string; competitorSummary?: string; displaySummary?: string; stockSummary?: string; demandSummary?: string; opportunitySummary?: string; riskSummary?: string; nextAction?: string };
type ReportCompetitor = { id: string; competitorName: string; brandName?: string | null; category?: string | null; area?: string | null };
type ReportProduct = { id: string; productId?: string | null; productName: string; brandName?: string | null; source?: string | null; note?: string | null };
type ReportContext = { loading: boolean; competitors: ReportCompetitor[]; usedProducts: ReportProduct[]; templates: ReportTemplate[] };
...
function emptyDraft(owner = ""): ActionDraft {
  return { productName: "", quantity: "1", unitPrice: "0", unit: "", note: "", reportType: "price", templateId: "", priceSummary: "", competitorSummary: "", displaySummary: "", stockSummary: "", demandSummary: "", opportunitySummary: "", riskSummary: "", nextAction: "", dueDate: "", priority: "medium", owner, skipReason: "", selectedCompetitorIds: [], selectedProductIds: [] };
}
...
async function fetchReportContext(sessionCustomerId: string): Promise<ReportContext> {
  ...
  return { loading: false, competitors: Array.isArray(data.competitors) ? data.competitors : [], usedProducts: Array.isArray(data.usedProducts) ? data.usedProducts : [], templates: Array.isArray(data.templates) ? data.templates : [] };
}
...
function ProductPickerPanel({ productSearch, saving, onSearchChange, onCategoryChange, onRunSearch, onPickProduct, onCommit, onClose }: { productSearch: ProductSearchState; saving: boolean; onSearchChange: (value: string) => void; onCategoryChange: (category: string) => void; onRunSearch: () => void; onPickProduct: (productId: string) => void; onCommit: (entries: PickerEntry[]) => void; onClose: () => void }) {
  ...
  function toggleVariant(variant: ProductCatalogItem) {
    setSelected((current) => {
      const next = { ...current };
      if (next[variant.variantId]) delete next[variant.variantId];
      else next[variant.variantId] = { variant, quantity: 1 };
      return next;
    });
  }

  function adjustQty(variant: ProductCatalogItem, delta: number) {
    setSelected((current) => {
      const existed = current[variant.variantId] || { variant, quantity: 0 };
      return { ...current, [variant.variantId]: { variant, quantity: Math.max(1, existed.quantity + delta) } };
    });
  }
  ...
  return (
    <div className="mcp-order-picker-panel" role="dialog" aria-modal="true" aria-label="Chọn sản phẩm">
      ...
      <div className="mcp-order-picker-tools">
        <div className="order-picker-head"><label className="form-field order-search-field"><small>Tìm tên / SKU</small><input value={productSearch.q} onChange={(event) => onSearchChange(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); onRunSearch(); } }} placeholder="Tìm tên sản phẩm, vị, SKU" disabled={saving || productSearch.loading} /></label><button className="button primary order-search-button" type="button" onClick={onRunSearch} disabled={saving || productSearch.loading}>{productSearch.loading ? "Tải..." : "Lọc"}</button></div>
        <div className="mcp-order-picker-filters"><button className={!productSearch.category ? "active" : ""} type="button" onClick={() => onCategoryChange("")} disabled={saving || productSearch.loading}>Tất cả</button>{categories.map((category) => <button className={productSearch.category === category ? "active" : ""} type="button" key={category} onClick={() => onCategoryChange(category)} disabled={saving || productSearch.loading}>{category}</button>)}</div>
      </div>
      {productSearch.error ? <p className="page-subtitle order-message">{productSearch.error}</p> : null}
      <div className="mcp-order-picker-body">
        <section className="mcp-picker-products"><div className="order-section-title"><strong>Loại sản phẩm / hãng</strong><span>{productSearch.loading ? "Đang tải..." : `${products.length} mục`}</span></div><div className="product-card-list compact">{products.length === 0 && !productSearch.loading ? <small className="page-subtitle">Không có sản phẩm phù hợp.</small> : null}{products.map((item) => <button className={productSearch.selectedProductId === item.productId ? "product-card active" : "product-card"} type="button" key={item.productId} onClick={() => onPickProduct(item.productId)} disabled={saving || productSearch.variantsLoading}><strong>{item.name}</strong><span>{item.brand || "Chưa brand"} · {item.category || "Chưa nhóm"}</span><small>{productSearch.results.filter((variant) => variant.productId === item.productId).length} quy cách · {item.sku}</small></button>)}</div></section>
        <section className="mcp-picker-variants"><div className="order-section-title"><strong>Vị / quy cách</strong><span>{productSearch.variantsLoading ? "Đang tải..." : `${productSearch.variants.length}`}</span></div><div className="variant-grid compact">{productSearch.variantsLoading ? <small className="page-subtitle">Đang tải quy cách...</small> : null}{!productSearch.variantsLoading && productSearch.selectedProductId && productSearch.variants.length === 0 ? <small className="page-subtitle">Sản phẩm này chưa có biến thể active.</small> : null}{productSearch.variants.map((variant) => { const entry = selected[variant.variantId]; return <article className={entry ? "mcp-picker-variant selected" : "mcp-picker-variant"} key={variant.variantId}><button type="button" onClick={() => toggleVariant(variant)} disabled={saving}><b>{entry ? "☑" : "□"} {variantLabel(variant)}</b><small>{variant.sellUnit || "đv"} · {formatMoney(Number(variant.price || 0))}</small></button><div className="mcp-picker-qty"><button type="button" onClick={() => adjustQty(variant, -1)} disabled={saving}>−</button><span>{entry?.quantity || 1}</span><button type="button" onClick={() => adjustQty(variant, 1)} disabled={saving}>+</button></div></article>; })}</div></section>
      </div>
      <div className="mcp-order-picker-foot"><span>Đã chọn: <strong>{selectedCount}</strong> mã</span><button className="button primary" type="button" onClick={() => onCommit(Object.values(selected))} disabled={saving || selectedCount === 0}>Thêm {selectedCount} mã vào đơn</button></div>
    </div>
  );
}
...
function OrderFields({ draft, productSearch, orderItems, orderTotal, saving, onChange, onSearchChange, onCategoryChange, onRunSearch, onPickProduct, onCommitPickerItems, onRemoveItem, onChangeItemQuantity }: { draft: ActionDraft; productSearch: ProductSearchState; orderItems: OrderDraftItem[]; orderTotal: number; saving: boolean; onChange: (field: keyof ActionDraft, value: string) => void; onSearchChange: (value: string) => void; onCategoryChange: (category: string) => void; onRunSearch: () => void; onPickProduct: (productId: string) => void; onCommitPickerItems: (entries: PickerEntry[]) => void; onRemoveItem: (variantId: string) => void; onChangeItemQuantity: (variantId: string, quantity: number) => void }) {
  ...
  return (
    <div className="order-builder order-builder-report-style">
      <button className="button primary mcp-order-open-picker" type="button" onClick={() => setPickerOpen(true)} disabled={saving}>+ Chọn sản phẩm</button>
      <div className="order-summary-list report-style">
        {orderItems.length === 0 ? <small className="page-subtitle">Chưa có item trong đơn. Bấm + Chọn sản phẩm để thêm nhiều mã một lần.</small> : null}
        {orderItems.map((item) => <div className="order-summary-row" key={item.variantId}><div><strong>{item.name}</strong><small>{variantLabel(item)} · {item.sku || item.variantId}</small></div><label><small>SL</small><input inputMode="decimal" value={item.quantity} onChange={(event) => onChangeItemQuantity(item.variantId, toNumber(event.target.value, item.quantity))} disabled={saving} /></label><span>{item.unit || item.sellUnit || "đv"} × {formatMoney(item.unitPrice)}</span><b>{formatMoney(item.lineTotal)}</b><button className="button" type="button" onClick={() => onRemoveItem(item.variantId)} disabled={saving}>Xóa</button></div>)}
      </div>
      <label className="form-field order-note-field"><small>Ghi chú giao hàng</small><textarea value={draft.note} onChange={(event) => onChange("note", event.target.value)} placeholder="Giao hàng / công nợ / thời gian giao" /></label>
      <div className="order-total-row"><span>Tổng</span><strong>{formatMoney(orderTotal)}</strong></div>
      {pickerOpen ? <ProductPickerPanel productSearch={productSearch} saving={saving} onSearchChange={onSearchChange} onCategoryChange={onCategoryChange} onRunSearch={onRunSearch} onPickProduct={onPickProduct} onCommit={(entries) => { onCommitPickerItems(entries); setPickerOpen(false); }} onClose={() => setPickerOpen(false)} /> : null}
    </div>
  );
}

function ReportContextFields({ draft, reportContext, saving, onChange, onToggleCompetitor, onToggleProduct, onApplyTemplate }: { draft: ActionDraft; reportContext: ReportContext; saving: boolean; onChange: (field: keyof ActionDraft, value: string) => void; onToggleCompetitor: (id: string) => void; onToggleProduct: (id: string) => void; onApplyTemplate: (id: string) => void }) {
  const templates = reportContext.templates.filter((template) => reportTemplateMatches(template, draft.reportType));
  return <div className="grid"><label className="form-field"><small>Loại báo cáo</small><select value={draft.reportType} onChange={(event) => onChange("reportType", event.target.value)} disabled={saving}><option value="price">Giá</option><option value="competitor">Đối thủ</option><option value="display">Trưng bày</option><option value="stock">Tồn kho</option><option value="demand">Nhu cầu</option></select></label><label className="form-field"><small>Chọn mẫu báo cáo</small><select value={draft.templateId} onChange={(event) => onApplyTemplate(event.target.value)} disabled={saving || reportContext.loading}><option value="">{reportContext.loading ? "Đang tải mẫu..." : "Không dùng mẫu"}</option>{templates.map((template) => <option key={template.id} value={template.id}>{template.title} · {template.scopeType || "global"}</option>)}</select></label><label className="form-field"><small>Nội dung báo cáo</small><textarea value={draft.note} onChange={(event) => onChange("note", event.target.value)} placeholder="Nội dung chính của báo cáo" /></label><label className="form-field"><small>Giá</small><input value={draft.priceSummary} onChange={(event) => onChange("priceSummary", event.target.value)} /></label><label className="form-field"><small>Đối thủ</small><input value={draft.competitorSummary} onChange={(event) => onChange("competitorSummary", event.target.value)} /></label><label className="form-field"><small>Trưng bày</small><input value={draft.displaySummary} onChange={(event) => onChange("displaySummary", event.target.value)} /></label><label className="form-field"><small>Tồn kho</small><input value={draft.stockSummary} onChange={(event) => onChange("stockSummary", event.target.value)} /></label><label className="form-field"><small>Nhu cầu</small><input value={draft.demandSummary} onChange={(event) => onChange("demandSummary", event.target.value)} /></label><label className="form-field"><small>Cơ hội</small><input value={draft.opportunitySummary} onChange={(event) => onChange("opportunitySummary", event.target.value)} /></label><label className="form-field"><small>Rủi ro</small><input value={draft.riskSummary} onChange={(event) => onChange("riskSummary", event.target.value)} /></label><label className="form-field"><small>Next action</small><input value={draft.nextAction} onChange={(event) => onChange("nextAction", event.target.value)} /></label><div className="visit-focus-card"><span>Đối thủ chọn nhanh</span><strong>{reportContext.loading ? "Đang tải..." : `${reportContext.competitors.length} lựa chọn`}</strong><div className="sheet-action-grid">{reportContext.competitors.map((item) => <label className="button" key={item.id}><input type="checkbox" checked={draft.selectedCompetitorIds.includes(item.id)} onChange={() => onToggleCompetitor(item.id)} disabled={saving} /> {item.competitorName}{item.category ? ` · ${item.category}` : ""}</label>)}</div></div><div className="visit-focus-card"><span>Sản phẩm đang dùng / đã mua / đã test</span><strong>{reportContext.loading ? "Đang tải..." : `${reportContext.usedProducts.length} sản phẩm`}</strong><div className="sheet-action-grid">{reportContext.usedProducts.map((item) => <label className="button" key={item.id}><input type="checkbox" checked={draft.selectedProductIds.includes(item.id)} onChange={() => onToggleProduct(item.id)} disabled={saving} /> {item.productName} · {productSourceLabel(item.source)}</label>)}</div></div></div>;
}

function ActionFields({ action, draft, productSearch, orderItems, orderTotal, reportContext, saving, onChange, onSearchChange, onCategoryChange, onRunSearch, onPickProduct, onCommitPickerItems, onRemoveOrderItem, onChangeItemQuantity, onToggleCompetitor, onToggleProduct, onApplyTemplate }: { action: McpCustomerAction; draft: ActionDraft; productSearch: ProductSearchState; orderItems: OrderDraftItem[]; orderTotal: number; reportContext: ReportContext; saving: boolean; onChange: (field: keyof ActionDraft, value: string) => void; onSearchChange: (value: string) => void; onCategoryChange: (category: string) => void; onRunSearch: () => void; onPickProduct: (productId: string) => void; onCommitPickerItems: (entries: PickerEntry[]) => void; onRemoveOrderItem: (variantId: string) => void; onChangeItemQuantity: (variantId: string, quantity: number) => void; onToggleCompetitor: (id: string) => void; onToggleProduct: (id: string) => void; onApplyTemplate: (id: string) => void }) {
...
export function McpSessionCompactView({ activeHref = "/visits", mcpDayData }: { activeHref?: string; routesData: RoutesData; mcpDayData: McpDayData; routeCustomersData: RouteCustomersData }) {
  ...
  function toggleDraftList(field: "selectedCompetitorIds" | "selectedProductIds", id: string) { setDraft((current) => ({ ...current, [field]: current[field].includes(id) ? current[field].filter((value) => value !== id) : [...current[field], id] })); }
  function applyReportTemplate(id: string) { const template = reportContext.templates.find((item) => item.id === id); if (!template) return setDraft((current) => ({ ...current, templateId: "" })); setDraft((current) => ({ ...current, templateId: template.id, reportType: template.reportType || current.reportType, note: template.content ?? current.note, priceSummary: template.priceSummary ?? current.priceSummary, competitorSummary: template.competitorSummary ?? current.competitorSummary, displaySummary: template.displaySummary ?? current.displaySummary, stockSummary: template.stockSummary ?? current.stockSummary, demandSummary: template.demandSummary ?? current.demandSummary, opportunitySummary: template.opportunitySummary ?? current.opportunitySummary, riskSummary: template.riskSummary ?? current.riskSummary, nextAction: template.nextAction ?? current.nextAction })); }

  async function loadProducts(q: string, category: string) { setMessage(null); setProductSearch((current) => ({ ...current, q, category, loading: true, error: null, results: [], variants: [], selectedProductId: "", selectedVariant: null })); try { const results = await fetchProductSearch(q.trim(), category); setProductSearch((current) => ({ ...current, q, category, loading: false, results, error: null })); } catch (error) { setProductSearch((current) => ({ ...current, q, category, loading: false, error: error instanceof Error ? error.message : "Không tìm được sản phẩm" })); } }
  ...
  async function pickProduct(productId: string) { const fallbackVariants = productSearch.results.filter((item) => item.productId === productId); setMessage(null); setProductSearch((current) => ({ ...current, selectedProductId: productId, variants: fallbackVariants, variantsLoading: true, error: null })); try { const variants = await fetchProductVariants(productId); setProductSearch((current) => ({ ...current, variantsLoading: false, variants: variants.length ? variants : fallbackVariants })); } catch (error) { setProductSearch((current) => ({ ...current, variantsLoading: false, error: error instanceof Error ? error.message : "Không tải được quy cách sản phẩm" })); } }

  function commitPickerItems(entries: PickerEntry[]) {
    if (entries.length === 0) return;
    setOrderItems((current) => {
      const next = [...current];
      entries.forEach(({ variant, quantity }) => {
        const unitPrice = Number(variant.price || 0);
        const unit = variant.sellUnit || "";
        const existed = next.find((item) => item.variantId === variant.variantId && item.unitPrice === unitPrice);
        if (existed) { existed.quantity += quantity; existed.lineTotal = existed.quantity * existed.unitPrice; }
        else next.push({ ...variant, quantity, unit, unitPrice, lineTotal: quantity * unitPrice });
      });
      return next;
    });
    setMessage(`Đã thêm ${entries.length} mã vào đơn`);
  }
  ...
  async function loadReportContextForLine(line: McpDayLine) { const sessionCustomerId = line.sessionCustomerId || line.id; setReportContext({ ...emptyReportContext(), loading: true }); try { setReportContext(await fetchReportContext(sessionCustomerId)); } catch (error) { setReportContext(emptyReportContext()); setMessage(error instanceof Error ? error.message : "Không tải được ngữ cảnh báo cáo"); } }
  function openCustomerAction(line: McpDayLine, action: McpCustomerAction) { setMessage(null); setSelectedLine(null); setDraft(emptyDraft(run.owner || "")); setProductSearch(emptyProductSearchState()); setOrderItems([]); setReportContext(emptyReportContext()); setSelectedAction({ line, action }); if (action === "order") void loadProducts("", ""); if (action === "market_report") void loadReportContextForLine(line); }
  function selectedReportCompetitors() { return reportContext.competitors.filter((item) => draft.selectedCompetitorIds.includes(item.id)); }
  function selectedReportProducts() { return reportContext.usedProducts.filter((item) => draft.selectedProductIds.includes(item.id)); }
  ...
  function submitAction() {
    ...
    startSaving(() => { void (async () => {
      try {
        setMessage(null);
        if (selectedAction.action === "order") { if (orderItems.length === 0) throw new Error("Cần thêm ít nhất 1 sản phẩm vào đơn"); await postMcpBackend("/api/mcp-orders/from-session-customer", { sessionCustomerId, items: orderItems.map((item) => ({ productId: item.productId, variantId: item.variantId, productName: item.name, sku: item.sku, unit: item.unit, quantity: item.quantity, unitPrice: item.unitPrice, note: variantLabel(item) })), note: draft.note, status: "confirmed" }); }
        else if (selectedAction.action === "test") { if (!draft.productName.trim()) throw new Error("Cần nhập sản phẩm test"); await postMcpBackend("/api/backend/mcp-day/session-customer/test", { sessionCustomerId, fileTitle: "Test nhanh từ checklist", results: [{ productName: draft.productName, status: draft.priority || "tested", note: draft.note }], note: draft.note, status: "tested" }); }
        else if (selectedAction.action === "market_report") { const competitorNames = selectedReportCompetitors().map((item) => item.competitorName); const productNames = selectedReportProducts().map((item) => `${item.productName} (${productSourceLabel(item.source)})`); const content = joinUnique([draft.note, productNames.length ? `Sản phẩm liên quan: ${productNames.join(", ")}` : "", competitorNames.length ? `Đối thủ liên quan: ${competitorNames.join(", ")}` : ""]); const competitorSummary = joinUnique([draft.competitorSummary, ...competitorNames]); const demandSummary = joinUnique([draft.demandSummary, productNames.length ? `Sản phẩm đã tick: ${productNames.join(", ")}` : ""]); if (!content && !draft.priceSummary && !competitorSummary && !draft.displaySummary && !draft.stockSummary && !demandSummary && !draft.opportunitySummary && !draft.riskSummary && !draft.nextAction) throw new Error("Cần nhập nội dung, chọn mẫu hoặc tick dữ liệu báo cáo"); await postMcpBackend("/api/backend/mcp-day/session-customer/report", { sessionCustomerId, reportType: draft.reportType, content, priceSummary: draft.priceSummary, competitorSummary, displaySummary: draft.displaySummary, stockSummary: draft.stockSummary, demandSummary, opportunitySummary: draft.opportunitySummary, riskSummary: draft.riskSummary, nextAction: draft.nextAction || content, templateId: draft.templateId || undefined, selectedCompetitors: selectedReportCompetitors(), selectedProducts: selectedReportProducts() }); }
        else if (selectedAction.action === "follow_up") { if (!draft.productName.trim()) throw new Error("Cần nhập tiêu đề follow-up"); await postMcpBackend("/api/backend/mcp-day/session-customer/followup", { sessionCustomerId, title: draft.productName, dueDate: draft.dueDate || undefined, priority: draft.priority, owner: draft.owner, note: draft.note, followupType: "general" }); }
        else if (selectedAction.action === "skip") { if (!draft.skipReason.trim()) throw new Error("Cần chọn lý do bỏ qua / không mua"); await postMcpBackend("/api/backend/mcp-day/session-customer/status", { sessionCustomerId, visitStatus: "skipped", statusReason: draft.skipReason, note: draft.note || draft.skipReason }); }
        setSelectedAction(null); setSelectedLine(null); router.refresh();
      } catch (error) { setMessage(error instanceof Error ? error.message : "Không lưu được hành động MCP"); }
    })(); });
  }

  return <AppShell activeHref={activeHref}><PageHeader eyebrow="Checklist phiên" title="Checklist phiên" subtitle={`Tuyến gốc: ${run.routeName} · Ngày: ${run.date} · Sale: ${run.owner}`} /><section className="mcp-gate-banner mcp-session-compact-head"><strong>{counters.pending} chờ ghé</strong><span>{counters.visited} đã ghé · {counters.skipped} bỏ qua · {counters.added} phát sinh · {counters.followups} follow-up · mở lúc {run.openedAt}</span></section><div className="mcp-status-chips" role="tablist" aria-label="Checklist phiên"><button className={tab === "all" ? "active" : ""} type="button" onClick={() => setTab("all")}>Tất cả khách <b>{counters.all}</b></button><button className={tab === "pending" ? "active" : ""} type="button" onClick={() => setTab("pending")}>Chờ ghé <b>{counters.pending}</b></button><button className={tab === "visited" ? "active" : ""} type="button" onClick={() => setTab("visited")}>Đã ghé <b>{counters.visited}</b></button><button className={tab === "skipped" ? "active" : ""} type="button" onClick={() => setTab("skipped")}>Bỏ qua <b>{counters.skipped}</b></button><button className={tab === "added" ? "active" : ""} type="button" onClick={() => setTab("added")}>Phát sinh <b>{counters.added}</b></button><button className={tab === "followups" ? "active" : ""} type="button" onClick={() => setTab("followups")}>Có follow-up <b>{counters.followups}</b></button></div><LineList lines={linesByTab[tab]} onOpen={setSelectedLine} onAction={openCustomerAction} /><CustomerSheet line={selectedLine} onClose={() => setSelectedLine(null)} onAction={openCustomerAction} /><CustomerActionSheet selection={selectedAction} draft={draft} productSearch={productSearch} orderItems={orderItems} orderTotal={orderTotal} reportContext={reportContext} saving={saving} message={message} onChange={updateDraft} onSearchChange={(value) => setProductSearch((current) => ({ ...current, q: value }))} onCategoryChange={changeProductCategory} onRunSearch={runProductSearch} onPickProduct={pickProduct} onCommitPickerItems={commitPickerItems} onRemoveOrderItem={removeOrderItem} onChangeItemQuantity={changeItemQuantity} onToggleCompetitor={(id) => toggleDraftList("selectedCompetitorIds", id)} onToggleProduct={(id) => toggleDraftList("selectedProductIds", id)} onApplyTemplate={applyReportTemplate} onClose={() => { if (!saving) setSelectedAction(null); }} onSubmit={submitAction} /></AppShell>;
}
