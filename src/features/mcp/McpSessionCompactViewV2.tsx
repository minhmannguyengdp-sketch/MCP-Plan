...
async function fetchProductSearch(q: string, category = "") {
...
function ProductPickerPanel({
  ...
  selected: PickerMap;
  saving: boolean;
  onSelectedChange: (value: PickerMap) => void;
  onSearchChange: (value: string) => void;
  onCategoryChange: (category: string) => void;
  onRunSearch: () => void;
  onPickProduct: (productId: string) => void;
  onCommit: (entries: PickerEntry[]) => void;
  onClose: () => void;
}) {
  const products = useMemo(
    () => productSearch.results.filter((item, index, values) => values.findIndex((candidate) => candidate.productId === item.productId) === index),
    [productSearch.results]
  );
  const categories = useMemo(
    () => Array.from(new Set([...DEFAULT_FILTERS, ...productSearch.results.map((item) => item.category || "").filter(Boolean)])),
    [productSearch.results]
  );
  const selectedEntries = useMemo(() => Object.values(selected), [selected]);
  const selectedByProduct = useMemo(() => {
    return selectedEntries.reduce<Record<string, number>>((acc, entry) => {
      acc[entry.variant.productId] = (acc[entry.variant.productId] || 0) + 1;
      return acc;
    }, {});
  }, [selectedEntries]);

  function variantsForProduct(productId: string) {
    return productSearch.results.filter((variant) => variant.productId === productId);
  }
  ...
  function adjustQty(variant: ProductCatalogItem, delta: number) {
    const existed = selected[variant.variantId] || { variant, quantity: 0 };
    onSelectedChange({
      ...selected,
      [variant.variantId]: { variant, quantity: Math.max(1, existed.quantity + delta) }
    });
  }

  function removeSelected(variantId: string) {
    const next = { ...selected };
    delete next[variantId];
    onSelectedChange(next);
  }

  function handleProductClick(item: ProductCatalogItem) {
    const variants = variantsForProduct(item.productId);
    if (variants.length <= 1) toggleVariant(variants[0] || item);
    else onPickProduct(item.productId);
  }

  return (
    <div className="mcp-order-picker-panel" role="dialog" aria-modal="true" aria-label="Chọn sản phẩm">
      <div className="mcp-order-picker-head">
        ...
        <div className="order-picker-head">
          <label className="form-field order-search-field">
            ...
            <input
              ...
              placeholder="Tìm tên sản phẩm, vị, SKU"
              disabled={saving || productSearch.loading}
            />
          </label>
          <button className="button primary order-search-button" type="button" onClick={onRunSearch} disabled={saving || productSearch.loading}>
            {productSearch.loading ? "Tải..." : "Lọc"}
          </button>
        </div>
        <div className="mcp-order-picker-filters">
          <button className={!productSearch.category ? "active" : ""} type="button" onClick={() => onCategoryChange("")} disabled={saving || productSearch.loading}>Tất cả</button>
          {categories.map((category) => (
            <button className={productSearch.category === category ? "active" : ""} type="button" key={category} onClick={() => onCategoryChange(category)} disabled={saving || productSearch.loading}>{category}</button>
          ))}
        </div>
      </div>
      ...
      <div className="mcp-order-picker-body">
        <section className="mcp-picker-products">
          <div className="order-section-title"><strong>Loại sản phẩm / hãng</strong><span>{productSearch.loading ? "Đang tải..." : `${products.length} mục`}</span></div>
          <div className="product-card-list compact">
            {products.length === 0 && !productSearch.loading ? <small className="page-subtitle">Không có sản phẩm phù hợp.</small> : null}
            {products.map((item) => {
              const variantCount = variantsForProduct(item.productId).length;
              const pickedCount = selectedByProduct[item.productId] || 0;
              const className = [productSearch.selectedProductId === item.productId ? "active" : "", pickedCount ? "selected" : ""].filter(Boolean).join(" ");
              return (
                <button className={`product-card ${className}`} type="button" key={item.productId} onClick={() => handleProductClick(item)} disabled={saving || productSearch.variantsLoading}>
                  <strong>{item.name}</strong>
                  <span>{item.brand || "Chưa brand"} · {item.category || "Chưa nhóm"}</span>
                  <small>{variantCount} quy cách · {item.sku}</small>
                  <em className="mcp-picker-product-badge">{pickedCount ? `Đã chọn ${pickedCount}` : variantCount <= 1 ? "+" : "Chọn vị"}</em>
                </button>
              );
            })}
          </div>
        ...
        <section className="mcp-picker-variants">
          <div className="order-section-title"><strong>Vị / quy cách</strong><span>{productSearch.variantsLoading ? "Đang tải..." : `${productSearch.variants.length}`}</span></div>
          <div className="variant-grid compact">
            {productSearch.variants.map((variant) => {
              const entry = selected[variant.variantId];
              return (
                <article className={entry ? "mcp-picker-variant selected" : "mcp-picker-variant"} key={variant.variantId}>
                  <button type="button" onClick={() => toggleVariant(variant)} disabled={saving}>
                    <b>{entry ? "☑" : "□"} {variantLabel(variant)}</b>
                    <small>{variant.sellUnit || "đv"} · {formatMoney(Number(variant.price || 0))}</small>
                  </button>
                  <div className="mcp-picker-qty">
                    <button type="button" onClick={() => adjustQty(variant, -1)} disabled={saving}>−</button>
      ...
      <div className="mcp-picker-selected-strip">
        {selectedEntries.length === 0 ? <small>Chưa chọn mã nào</small> : selectedEntries.map((entry) => (
          <span key={entry.variant.variantId}><b>{entry.quantity}×</b> {entry.variant.name} · {variantLabel(entry.variant)} <button type="button" onClick={() => removeSelected(entry.variant.variantId)} disabled={saving}>×</button></span>
        ))}
      </div>

      <div className="mcp-order-picker-foot">
        <span>Đã chọn: <strong>{selectedEntries.length}</strong> mã</span>
        <button className="button primary" type="button" onClick={() => onCommit(selectedEntries)} disabled={saving || selectedEntries.length === 0}>Thêm {selectedEntries.length} mã vào đơn</button>
      </div>
    </div>
  );
}
      ...
      <div className="order-summary-list report-style">
        ...
        {orderItems.map((item) => (
          <div className="order-summary-row" key={item.variantId}>
            <div><strong>{item.name}</strong><small>{variantLabel(item)} · {item.sku || item.variantId}</small></div>
            <label><small>SL</small><input inputMode="decimal" value={item.quantity} onChange={(event) => onChangeItemQuantity(item.variantId, toNumber(event.target.value, item.quantity))} disabled={saving} /></label>
            <span>{item.unit || item.sellUnit || "đv"} × {formatMoney(item.unitPrice)}</span>
...
function ActionFields({ action, draft, productSearch, orderItems, orderTotal, saving, onChange, onSearchChange, onCategoryChange, onRunSearch, onPickProduct, onCommitPickerItems, onRemoveOrderItem, onChangeItemQuantity }: { action: McpCustomerAction; draft: ActionDraft; productSearch: ProductSearchState; orderItems: OrderDraftItem[]; orderTotal: number; saving: boolean; onChange: (field: keyof ActionDraft, value: string) => void; onSearchChange: (value: string) => void; onCategoryChange: (category: string) => void; onRunSearch: () => void; onPickProduct: (productId: string) => void; onCommitPickerItems: (entries: PickerEntry[]) => void; onRemoveOrderItem: (variantId: string) => void; onChangeItemQuantity: (variantId: string, quantity: number) => void }) {
  ...
  return <div className="grid"><label className="form-field"><small>Nội dung báo cáo</small><textarea value={draft.note} onChange={(event) => onChange("note", event.target.value)} placeholder="Giá / đối thủ / tồn kho / nhu cầu" /></label></div>;
}

function CustomerActionSheet({ selection, draft, productSearch, orderItems, orderTotal, saving, message, onChange, onSearchChange, onCategoryChange, onRunSearch, onPickProduct, onCommitPickerItems, onRemoveOrderItem, onChangeItemQuantity, onClose, onSubmit }: { selection: { line: McpDayLine; action: McpCustomerAction } | null; draft: ActionDraft; productSearch: ProductSearchState; orderItems: OrderDraftItem[]; orderTotal: number; saving: boolean; message: string | null; onChange: (field: keyof ActionDraft, value: string) => void; onSearchChange: (value: string) => void; onCategoryChange: (category: string) => void; onRunSearch: () => void; onPickProduct: (productId: string) => void; onCommitPickerItems: (entries: PickerEntry[]) => void; onRemoveOrderItem: (variantId: string) => void; onChangeItemQuantity: (variantId: string, quantity: number) => void; onClose: () => void; onSubmit: () => void }) {
...
export function McpSessionCompactView({ activeHref = "/visits", mcpDayData }: { activeHref?: string; routesData: RoutesData; mcpDayData: McpDayData; routeCustomersData: RouteCustomersData }) {
  ...
  function openCustomerAction(line: McpDayLine, action: McpCustomerAction) { setMessage(null); setSelectedLine(null); setDraft(emptyDraft(run.owner || "")); setProductSearch(emptyProductSearchState()); setOrderItems([]); setSelectedAction({ line, action }); if (action === "order") void loadProducts("", ""); }
  function submitAction() { if (!selectedAction) return; const sessionCustomerId = selectedAction.line.sessionCustomerId || selectedAction.line.id; startSaving(() => { void (async () => { try { setMessage(null); if (selectedAction.action === "order") { if (orderItems.length === 0) throw new Error("Cần thêm ít nhất 1 sản phẩm vào đơn"); await postMcpBackend("/api/mcp-orders/from-session-customer", { sessionCustomerId, items: orderItems.map((item) => ({ productId: item.productId, variantId: item.variantId, productName: item.name, sku: item.sku, unit: item.unit, quantity: item.quantity, unitPrice: item.unitPrice, note: variantLabel(item) })), note: draft.note, status: "confirmed" }); } else if (selectedAction.action === "test") await postMcpBackend("/api/backend/mcp-day/session-customer/test", { sessionCustomerId, fileTitle: "Test nhanh từ checklist", results: [{ productName: draft.productName, status: draft.priority || "tested", note: draft.note }], note: draft.note, status: "tested" }); else if (selectedAction.action === "market_report") await postMcpBackend("/api/backend/mcp-day/session-customer/report", { sessionCustomerId, reportType: "general", content: draft.note, nextAction: draft.note }); else if (selectedAction.action === "follow_up") await postMcpBackend("/api/backend/mcp-day/session-customer/followup", { sessionCustomerId, title: draft.productName, dueDate: draft.dueDate || undefined, priority: draft.priority, owner: draft.owner, note: draft.note, followupType: "general" }); else if (selectedAction.action === "skip") await postMcpBackend("/api/backend/mcp-day/session-customer/status", { sessionCustomerId, visitStatus: "skipped", statusReason: draft.skipReason || "other", note: draft.note || draft.skipReason }); setSelectedAction(null); setSelectedLine(null); router.refresh(); } catch (error) { setMessage(error instanceof Error ? error.message : "Không lưu được hành động MCP"); } })(); }); }

  return <AppShell activeHref={activeHref}><PageHeader eyebrow="Checklist phiên" title="Checklist phiên" subtitle={`Tuyến gốc: ${run.routeName} · Ngày: ${run.date} · Sale: ${run.owner}`} /><section className="mcp-gate-banner mcp-session-compact-head"><strong>{counters.pending} chờ ghé</strong><span>{counters.visited} đã ghé · {counters.skipped} bỏ qua · {counters.added} phát sinh · {counters.followups} follow-up · mở lúc {run.openedAt}</span></section><div className="mcp-status-chips" role="tablist" aria-label="Checklist phiên"><button className={tab === "all" ? "active" : ""} type="button" onClick={() => setTab("all")}>Tất cả khách <b>{counters.all}</b></button><button className={tab === "pending" ? "active" : ""} type="button" onClick={() => setTab("pending")}>Chờ ghé <b>{counters.pending}</b></button><button className={tab === "visited" ? "active" : ""} type="button" onClick={() => setTab("visited")}>Đã ghé <b>{counters.visited}</b></button><button className={tab === "skipped" ? "active" : ""} type="button" onClick={() => setTab("skipped")}>Bỏ qua <b>{counters.skipped}</b></button><button className={tab === "added" ? "active" : ""} type="button" onClick={() => setTab("added")}>Phát sinh <b>{counters.added}</b></button><button className={tab === "followups" ? "active" : ""} type="button" onClick={() => setTab("followups")}>Có follow-up <b>{counters.followups}</b></button></div><LineList lines={linesByTab[tab]} onOpen={setSelectedLine} onAction={openCustomerAction} /><CustomerSheet line={selectedLine} onClose={() => setSelectedLine(null)} onAction={openCustomerAction} /><CustomerActionSheet selection={selectedAction} draft={draft} productSearch={productSearch} orderItems={orderItems} orderTotal={orderTotal} saving={saving} message={message} onChange={updateDraft} onSearchChange={(value) => setProductSearch((current) => ({ ...current, q: value }))} onCategoryChange={changeProductCategory} onRunSearch={runProductSearch} onPickProduct={pickProduct} onCommitPickerItems={commitPickerItems} onRemoveOrderItem={removeOrderItem} onChangeItemQuantity={changeItemQuantity} onClose={() => { if (!saving) setSelectedAction(null); }} onSubmit={submitAction} /></AppShell>;
}
