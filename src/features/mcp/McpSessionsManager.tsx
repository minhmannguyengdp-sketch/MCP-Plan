  ...
  function saveEdit() {
    ...
    startTransition(async () => {
      try {
        ...
        setMessage(error instanceof Error ? error.message : "Không cập nhật được phiên");
      }
    });
  }
  ...
  function deleteSession() {
    ...
    startTransition(async () => {
      try {
        ...
        setMessage(error instanceof Error ? error.message : "Không xóa được phiên");
      }
    });
  }
      ...
      <section className="grid mcp-session-list">
        {data.sessions.length === 0 ? <div className="empty-inline">Chưa có phiên trong bộ lọc.</div> : data.sessions.map((session) => (
          <article className="action-card mcp-session-card" key={session.id}>
            <div>
              <span className="badge">{statusLabels[session.status] || session.status}</span>
            ...
            <div className="mcp-session-card-actions">
              <Link className="button primary" href={`/visits?routeId=${encodeURIComponent(session.routeId)}&date=${encodeURIComponent(session.sessionDate)}`} prefetch>Mở checklist</Link>
              <button className="button" type="button" onClick={() => openEdit(session)}>Sửa phiên</button>
              <button className="button danger" type="button" onClick={() => openDelete(session)}>Xóa phiên</button>
            </div>
          </article>
        ))}
      </section>

      <BottomSheet open={Boolean(editing)} onClose={closeSheets} title="Sửa phiên" description={editing ? `${editing.routeName} · chỉ sửa ngày/trạng thái/ghi chú, không đổi tuyến gốc.` : undefined} footer={<div className="sheet-action-grid"><button className="button" type="button" onClick={closeSheets} disabled={pending}>Đóng</button><button className="button primary" type="button" onClick={saveEdit} disabled={pending}>{pending ? "Đang lưu..." : "Lưu phiên"}</button></div>}>
        {editing ? <div className="grid"><label className="form-field"><small>Ngày phiên</small><input type="date" value={draft.sessionDate} onChange={(event) => setDraft((current) => ({ ...current, sessionDate: event.target.value }))} /></label><label className="form-field"><small>Trạng thái</small><select value={draft.status} onChange={(event) => setDraft((current) => ({ ...current, status: event.target.value }))}><option value="active">Đang chạy</option><option value="done">Đã chốt</option><option value="cancelled">Đã hủy</option></select></label><label className="form-field"><small>Ghi chú</small><textarea value={draft.note} onChange={(event) => setDraft((current) => ({ ...current, note: event.target.value }))} placeholder="Lý do chỉnh/hủy phiên" /></label>{message ? <p className="page-subtitle order-message">{message}</p> : null}</div> : null}
      </BottomSheet>

      <BottomSheet open={Boolean(deleting)} onClose={closeSheets} title="Xóa phiên" description={deleting ? `${deleting.routeName} · ${deleting.sessionDate}` : undefined} footer={<div className="sheet-action-grid"><button className="button" type="button" onClick={closeSheets} disabled={pending}>Đóng</button><button className="button danger" type="button" onClick={deleteSession} disabled={pending}>{pending ? "Đang xóa..." : "Xóa phiên rỗng"}</button></div>}>
        {deleting ? <div className="visit-sheet-content"><div className="visit-focus-card"><span>Cảnh báo</span><strong>Chỉ xóa phiên chưa phát sinh hoạt động</strong><small>Nếu phiên đã có khách ghé, đơn, test, báo cáo hoặc follow-up, hệ thống sẽ chặn xóa. Khi đó hãy Sửa phiên → chuyển trạng thái Đã hủy.</small></div><div className="metric-row"><span>Khách đã ghé</span><strong>{deleting.visitedCustomers}/{deleting.plannedCustomers}</strong></div>{message ? <p className="page-subtitle order-message">{message}</p> : null}</div> : null}
      </BottomSheet>
    </>
  );
}
