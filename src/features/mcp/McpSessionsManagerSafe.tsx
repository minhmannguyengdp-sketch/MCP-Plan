...
export function McpSessionsManagerSafe({ data, filters }: { data: SessionsPayload; filters: { dateFrom: string; dateTo: string; routeId: string; status: string } }) {
  ...
  return <>
    ...
    <section className="grid mcp-session-list">{data.sessions.length === 0 ? <div className="empty-inline">Chưa có phiên trong bộ lọc.</div> : data.sessions.map((s) => <article className="action-card mcp-session-card" key={s.id}><div><span className="badge">{labels[s.status] || s.status}</span><h3>{s.routeName}</h3><p>{s.sessionDate} · {s.visitedCustomers}/{s.plannedCustomers} khách đã ghé</p></div><div className="mcp-session-card-actions"><Link className="button primary" href={`/visits?routeId=${encodeURIComponent(s.routeId)}&date=${encodeURIComponent(s.sessionDate)}`} prefetch>Mở checklist</Link><ExportMenu label="Xuất" groups={[{ title: "Phiên này", links: [buildExportLink("Excel checklist", sessionExcelUrl(s.id), "primary", "Dữ liệu dòng khách trong phiên"), buildExportLink("PDF báo cáo ngày", sessionPdfUrl(s.id), undefined, "Bản in theo phiên")]}]} /><button className="button" type="button" onClick={() => openEdit(s)}>Sửa phiên</button><span className="page-subtitle" style={{ alignSelf: "center", fontSize: 12 }}>Đang khóa xóa</span></div></article>)}</section>
