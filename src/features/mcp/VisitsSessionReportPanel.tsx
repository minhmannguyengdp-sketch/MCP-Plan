...
function buildUrl(mcpDayData: McpDayData) {
  ...
  return `/api/mcp-session-report?${params.toString()}`;
}
...
export function VisitsSessionReportPanel({ mcpDayData }: { mcpDayData: McpDayData }) {
  ...
  return <>
    <button type="button" style={buttonStyle} onClick={() => { setOpen(true); setSummary(null); }}>BC phiên</button>
    <BottomSheet open={open} onClose={() => setOpen(false)} title="BC phiên" description={`${mcpDayData.run.routeName} · ${mcpDayData.run.date}`} footer={<div className="sheet-action-grid"><button className="button" type="button" onClick={() => setOpen(false)}>Đóng</button><button className="button primary" type="button" onClick={() => { setSummary(null); setError(null); }}>Tải lại</button></div>}>
