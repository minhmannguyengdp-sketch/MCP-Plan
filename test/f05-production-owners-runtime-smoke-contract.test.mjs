...
function completePreflightProof() {
  return {
    remoteIdentity: { source: "authenticated-persisted-installation", installationId: "installation-a", nppCode: "NPP-A" },
    operationInvariantVerifiers: MUTATION_INVENTORY.map(({ name }) => name),
    archiveRetryReclaimVerifier: true,
    r2ProviderCleanupVerifier: true
  };
}

const expectedIdentity = { installationId: "installation-a", nppCode: "NPP-A" };
...
test("runner always cleans up and emits complete machine-readable PASS evidence", async () => {
  const calls = [];
  const driver = {
    async provePreflightCapabilities() { calls.push("preflight"); return completePreflightProof(); },
  ...
  const evidence = await runF05ProductionOwnersSmoke(driver, expectedIdentity);
  assert.equal(evidence.NPP_F05_PRODUCTION_OWNERS_RUNTIME_SMOKE, "PASS");
  assert.equal(evidence.preflight.status, "PASS");
...
test("runner executes cleanup after a primary failure and never upgrades incomplete evidence", async () => {
  ...
  await assert.rejects(runF05ProductionOwnersSmoke({
    async provePreflightCapabilities() { return completePreflightProof(); },
  ...
  }, expectedIdentity), /npp_f05_production_smoke_failed/);
...
test("runner rejects synthetic PASS labels, missing invariants, and DB-only cleanup", async () => {
  ...
  await assert.rejects(runF05ProductionOwnersSmoke({ ...base, async proveOperation() { return { execute: "PASS", replay: "PASS", conflict: "PASS" }; } }, expectedIdentity), /npp_f05_production_smoke_failed/);
  await assert.rejects(runF05ProductionOwnersSmoke({ ...base, async proveOperation() { return { ...completeOperationProof(), invariants: { verified: false } }; } }, expectedIdentity), /npp_f05_production_smoke_failed/);
  await assert.rejects(runF05ProductionOwnersSmoke({ ...base, async proveOperation() { return completeOperationProof(); }, async verifyCleanup() { return { databaseRowsRemaining: 0, r2ProviderVerified: false, r2ObjectsRemaining: null }; } }, expectedIdentity), /npp_f05_production_smoke_failed/);
});

test("unproven preflight capabilities fail before fixture creation or cleanup", async () => {
  const calls = [];
  let rejected;
  try {
    await runF05ProductionOwnersSmoke({
      async provePreflightCapabilities() {
        calls.push("preflight");
        return { ...completePreflightProof(), r2ProviderCleanupVerifier: false };
      },
      async createTemporaryFixtures() { calls.push("create"); },
      async cleanupTemporaryFixtures() { calls.push("cleanup"); }
    }, expectedIdentity);
  } catch (error) {
    rejected = error;
  ...
  assert.equal(rejected.evidence.NPP_F05_PRODUCTION_OWNERS_RUNTIME_SMOKE, "FAIL");
  assert.equal(rejected.evidence.preflight.status, "PENDING");
});

test("preflight remote identity must match the independently guarded identity", async () => {
  const calls = [];
  await assert.rejects(runF05ProductionOwnersSmoke({
    async provePreflightCapabilities() {
      calls.push("preflight");
      return {
        ...completePreflightProof(),
        remoteIdentity: { ...completePreflightProof().remoteIdentity, installationId: "other-installation" }
      };
    },
    async createTemporaryFixtures() { calls.push("create"); }
  }, expectedIdentity), /npp_f05_production_smoke_failed/);
...
test("live driver targets only captured temporary IDs and never prints provider keys", async () => {
  ...
  assert.match(driver, /provePreflightCapabilities/);
  assert.match(driver, /remoteIdentity: \{ source: "unverified"/);
