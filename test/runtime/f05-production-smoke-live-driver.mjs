...
export function createLiveF05SmokeDriver() {
  ...
  return {
    ...
    async proveArchiveLifecycle(fixtures) {
      ...
      const failureObserved = jobs.some((job) => Number(job.attempt_count) >= 2) || intents.some((intent) => Number(intent.failed_attempt_count) >= 1);
      ensure(failureObserved, "archive_retry_not_proven");
      const reclaimObserved = jobs.every((job) => job.claimed_at || Number(job.attempt_count) >= 1);
      ensure(reclaimObserved, "archive_reclaim_not_proven");
      return {
        sequence: [
          { stage: "failure", observed: failureObserved },
          { stage: "retry-claim", observed: jobs.some((job) => Number(job.attempt_count) >= 2) },
          { stage: "reclaim", observed: reclaimObserved },
          { stage: "completion", observed: jobs.every((job) => job.status === "completed" && Boolean(job.completed_at)) },
          { stage: "finalizer", observed: intents.every((intent) => intent.status === "completed" && Boolean(intent.finalized_at)) }
        ],
