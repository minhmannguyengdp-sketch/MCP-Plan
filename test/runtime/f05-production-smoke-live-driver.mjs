...
async function uploadProofMedia(routeCustomerId, sessionId, suffix) {
  const upload = await must("/api/outlet-media/upload-init", { method: "POST", idempotencyKey: `npp-f05.fixture.media.${suffix}.${stamp}`, body: { routeCustomerId, sessionId, clientUploadId: `npp-f05-${suffix}-${stamp}`, mimeType: "image/jpeg", byteSize: 4, mediaType: "storefront" } });
