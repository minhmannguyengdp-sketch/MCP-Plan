...
export function createLiveF05SmokeDriver() {
  ...
  return {
    async provePreflightCapabilities() {
      // Do not create even temporary production fixtures until every proof owner exists.
      // The live implementations for persisted business invariants, forced archive
      // retry/reclaim, provider-level R2 absence, and server-observed identity are not
      // present yet, so this driver must stop before its first network mutation.
      return {
        remoteIdentity: { source: "unverified", installationId: "", nppCode: "" },
        operationInvariantVerifiers: [],
        archiveRetryReclaimVerifier: false,
        r2ProviderCleanupVerifier: false
      };
    },
